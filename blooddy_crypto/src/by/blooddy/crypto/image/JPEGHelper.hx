////////////////////////////////////////////////////////////////////////////////
//
//  © 2010 BlooDHounD
//
////////////////////////////////////////////////////////////////////////////////

package by.blooddy.crypto.image;

import by.blooddy.system.Memory;
import flash.utils.ByteArray;

/**
 * @author	BlooDHounD
 * @version	1.0
 */
class JPEGHelper {

	public static function createQuantTable(?quality:UInt=60):ByteArray {
		return TMP.createQuantTables( quality );
	}

	public static function createHuffmanTable():ByteArray {
		return TMP.createHuffmanTable();
	}
}

/**
 * @private
 */
private class TMP {

	public static inline var O:UInt = 64 + 64 + 512 + 512;

	public static function createQuantTables(quality:UInt):ByteArray {

		var sf:UInt = quality < 50 ? Std.int( 5000 / quality ) : Std.int( 200 - ( quality << 1 ) );

		var mem:ByteArray = Memory.memory;

		// YTable[1,64], UVTable[1,64], fdtbl_Y[8,64], fdtbl_UV[8,64]
		var bytes:ByteArray = new ByteArray();

		bytes.position = 128;
		// YQT [64..128]=1
		bytes.writeUTFBytes( '' );
		// UVQT [128..192]=1
		bytes.writeUTFBytes( '' );

		// ZigZag [0..64]=1
		bytes.position = O;
		bytes.writeUTFBytes( '' );

		bytes.length += 64;

		Memory.memory = bytes;
		
		var i:UInt;
		var t:UInt;

		// YTable
		i = 0;
		do {
			t = Std.int( ( Memory.getByte( 128 + i ) * sf + 50 ) / 100 );
			if ( t < 1 ) t = 1;
			else if ( t > 255 ) t = 255;
			Memory.setByte( Memory.getByte( O + i ), t );
		} while ( ++i < 64 );

		// UVTable
		i = 0;
		do {
			t = Std.int( ( Memory.getByte( 192 + i ) * sf + 50 ) / 100 );
			if ( t < 1 ) t = 1;
			else if ( t > 255 ) t = 255;
			Memory.setByte( 64 + Memory.getByte( O + i ), t );
		} while ( ++i < 64 );

		// aasf [64..128]=8
		Memory.setDouble( O + 64 + 8 * 0, 1.000000000 );
		Memory.setDouble( O + 64 + 8 * 1, 1.387039845 );
		Memory.setDouble( O + 64 + 8 * 2, 1.306562965 );
		Memory.setDouble( O + 64 + 8 * 3, 1.175875602 );
		Memory.setDouble( O + 64 + 8 * 4, 1.000000000 );
		Memory.setDouble( O + 64 + 8 * 5, 0.785694958 );
		Memory.setDouble( O + 64 + 8 * 6, 0.541196100 );
		Memory.setDouble( O + 64 + 8 * 7, 0.275899379 );

		// fdtbl_Y
		// fdtbl_UV
		var row:UInt;
		var col:UInt;
		var n:Float;
		i = 0;
		row = 0;
		do {
			col = 0;
			do {
				n = Memory.getDouble( O + 64 + row ) * Memory.getDouble( O + 64 + col ) * 8;
				Memory.setDouble( 128       + i * 8, 1.0 / ( Memory.getByte(      Memory.getByte( O + i ) ) * n ) );
				Memory.setDouble( 128 + 512 + i * 8, 1.0 / ( Memory.getByte( 64 + Memory.getByte( O + i ) ) * n ) );
				++i;
				col += 8;
			} while ( col < 64 );
			row += 8;
		} while ( row < 64 );

		bytes.length = O;
		bytes.position = 0;
		Memory.memory = mem;

		return bytes;
	}

	public static function createHuffmanTable():ByteArray {

		var mem:ByteArray = Memory.memory;

		// 0
		// std_dc_luminance_nrcodes[1,17],
		// std_dc_luminance_values[1,12],
		// 0
		// std_ac_luminance_nrcodes[1,17],
		// std_ac_luminance_values,
		// 0
		// std_dc_chrominance_nrcodes[1,17],
		// std_dc_chrominance_values[1,12],
		// 0
		// std_ac_chrominance_nrcodes[1,17],
		// std_ac_chrominance_values
		var bytes:ByteArray = new ByteArray();
		bytes.length = 1024;

		Memory.memory = bytes;

		var i:Int;
		var arr:Array<Int>;

		// std_dc_luminance_nrcodes		[0..16]=1
		bytes.position++;
		bytes.writeUTFBytes( '\x00\x01\x05\x01\x01\x01\x01\x01\x01\x00\x00\x00\x00\x00\x00\x00'	);
		// std_dc_luminance_values		[0..12]=1
		bytes.writeUTFBytes( '\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b'	);
		// std_ac_luminance_nrcodes		[0..16]=1
		bytes.position++;
		bytes.writeUTFBytes( '\x00\x02\x01\x03\x03\x02\x04\x03\x05\x05\x04\x04\x00\x00\x01\x7d'	);
		// std_ac_luminance_values		[0..162]=1
		arr = [ 0x00030201, 0x12051104, 0x06413121, 0x07615113, 0x32147122, 0x08a19181, 0xc1b14223, 0xf0d15215, 0x72623324, 0x160a0982, 0x1a191817, 0x28272625, 0x35342a29, 0x39383736, 0x4544433a, 0x49484746, 0x5554534a, 0x59585756, 0x6564635a, 0x69686766, 0x7574736a, 0x79787776, 0x8584837a, 0x89888786, 0x9493928a, 0x98979695, 0xa3a29a99, 0xa7a6a5a4, 0xb2aaa9a8, 0xb6b5b4b3, 0xbab9b8b7, 0xc5c4c3c2, 0xc9c8c7c6, 0xd4d3d2ca, 0xd8d7d6d5, 0xe2e1dad9, 0xe6e5e4e3, 0xeae9e8e7, 0xf4f3f2f1, 0xf8f7f6f5, 0x0000faf9 ];
		i = 0;
		do {
			Memory.setI32( 46 + i * 4, arr[ i ] );
		} while ( ++i < 41 );

		bytes.position = 208;
		// std_dc_chrominance_nrcodes	[0..16]=1
		bytes.position++;
		bytes.writeUTFBytes( '\x00\x03\x01\x01\x01\x01\x01\x01\x01\x01\x01\x00\x00\x00\x00\x00'	);
		// std_dc_chrominance_values	[0..12]=1
		bytes.writeBytes( bytes, 17, 12 );
		// std_ac_chrominance_nrcodes	[0..16]=1
		bytes.position++;
		bytes.writeUTFBytes( '\x00\x02\x01\x02\x04\x04\x03\x04\x07\x05\x04\x04\x00\x01\x02\x77'	);
		// std_ac_chrominance_values	[0..162]=1
		arr = [ 0x03020100, 0x21050411, 0x41120631, 0x71610751, 0x81322213, 0x91421408, 0x09c1b1a1, 0xf0523323, 0xd1726215, 0x3424160a, 0x17f125e1, 0x261a1918, 0x2a292827, 0x38373635, 0x44433a39, 0x48474645, 0x54534a49, 0x58575655, 0x64635a59, 0x68676665, 0x74736a69, 0x78777675, 0x83827a79, 0x87868584, 0x928a8988, 0x96959493, 0x9a999897, 0xa5a4a3a2, 0xa9a8a7a6, 0xb4b3b2aa, 0xb8b7b6b5, 0xc3c2bab9, 0xc7c6c5c4, 0xd2cac9c8, 0xd6d5d4d3, 0xdad9d8d7, 0xe5e4e3e2, 0xe9e8e7e6, 0xf4f3f2ea, 0xf8f7f6f5, 0x0000faf9 ];
		i = 0;
		do {
			Memory.setI32( 254 + i * 4, arr[ i ] );
		} while ( ++i < 41 );

		bytes.length = 416;
		bytes.position = 0;
		Memory.memory = mem;

		return bytes;
	}

}